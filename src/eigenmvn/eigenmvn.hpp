...
namespace Eigen {
  ...
  */
  ...
  public:
  mvrnorm(const std::vector<Scalar>& mean,const Matrix<Scalar,Dynamic,Dynamic>& covar,
			  const bool use_cholesky=true,const uint64_t &seed=std::mt19937::default_seed)
    ...
    void setMean(const std::vector<Scalar>& mean) {
    	int n = mean.size();
    	_mean.resize(n,1);
    	for(int i = 0; i < n; i++){
    		_mean(i,0) = mean[i];
    ...
    Matrix<Scalar,Dynamic,-1> sample(int nn)
      {
